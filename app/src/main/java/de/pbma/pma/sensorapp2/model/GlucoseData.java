...
import static de.pbma.pma.sensorapp2.SensorApp.MainActivity.GLUCOSE_UNIT_IS_MMOL;
...
public class GlucoseData implements Comparable<GlucoseData> {
    ...
    public GlucoseData(SensorData sensor, int ageInSensorMinutes, int timezoneOffsetInMinutes, int glucoseLevelRaw, boolean isTrendData, long date) {
        this.sensor = sensor;
        this.ageInSensorMinutes = ageInSensorMinutes; //verified --> Minuten seit Sensor gestartet wurde
        this.timezoneOffsetInMinutes = timezoneOffsetInMinutes;
        this.glucoseLevelRaw = glucoseLevelRaw; //525 im Beispiel
        this.isTrendData = isTrendData;
        this.date = date; //1538563421532
